...
export default function AlertFeed() {
  const { alerts, notificationsMuted, alertSeverity } = useMemeStore();
  const [toasts, setToasts] = useState([]);
  const [prevLength, setPrevLength] = useState(0);

  const severityThreshold = severityOrder[alertSeverity] ?? 0;
  const visibleAlerts = useMemo(() => {
    return alertSeverity === 'all'
      ? alerts
      : alerts.filter((alert) => (severityOrder[alert.alert_level] ?? 0) >= severityThreshold);
  }, [alerts, alertSeverity, severityThreshold]);

  useEffect(() => {
    if (!notificationsMuted && visibleAlerts.length > prevLength) {
      const newAlerts = visibleAlerts.slice(0, visibleAlerts.length - prevLength);
      setToasts(prev => [...newAlerts, ...prev].slice(0, 3));
    }
    setPrevLength(visibleAlerts.length);
  }, [notificationsMuted, visibleAlerts]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] text-muted font-mono uppercase tracking-widest">
                Real-time Signals {alertSeverity === 'all' ? '' : `| ${alertSeverity} and above`}
              </span>
            </div>
          </div>
          <div className="px-3 py-1 rounded-full bg-white/5 border border-white/10">
            <span className="text-[10px] font-black text-muted tracking-tighter">{visibleAlerts.length} ALERTS</span>
          </div>
        </div>
