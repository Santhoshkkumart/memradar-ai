...
export default function GlareCard({ children, className }) {
  ...
  return (
    <div
      ...
      onPointerEnter={() => {
        ...
        if (refElement.current) {
          refElement.current.style.setProperty('--opacity', '0.55');
          setTimeout(() => {
    ...
    >
      <div className={cn('relative w-full overflow-hidden rounded-[var(--radius)] border border-white/10 bg-slate-950', className)}>
        <div className="absolute inset-0 opacity-[var(--opacity)] transition-opacity duration-[var(--duration)] ease-[var(--easing)] [background:radial-gradient(farthest-corner_circle_at_var(--m-x)_var(--m-y),_rgba(255,255,255,0.25)_10%,_rgba(255,255,255,0.1)_20%,_rgba(255,255,255,0)_90%)]" />
        <div
          className="absolute inset-0 opacity-[var(--opacity)] mix-blend-color-dodge transition-opacity [background:var(--pattern),_var(--rainbow),_var(--diagonal),_var(--shade)]"
